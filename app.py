...
def run_ffmpeg(video_path, stream_key, log_callback, is_shorts=False, custom_rtmp=None, session_id=None):
    ...
    cmd = [
        ...
        "-c:v", "libx264", "-preset", "veryfast", "-b:v", v_bitrate,
        "-maxrate", v_bitrate, "-bufsize", buf_size,
        "-pix_fmt", "yuv420p", "-g", "60", "-c:a", "aac", "-b:a", "128k", "-ar", "44100",
        "-f", "flv", output_url
    ]
    
    start_msg = f"🚀 Starting Optimized Stream ({width}x{height}, {v_bitrate})..."
...
def main():
    ...
    if not st.session_state['authenticated']:
        st.markdown("<h1 style='text-align: center;'>🔐 Login Panel</h1>", unsafe_allow_html=True)
        ...
        with col2:
            with st.form("login_form"):
                ...
                if submit:
                    if username == "admin" and password == "stream-YT":
